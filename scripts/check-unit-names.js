 ...
 * already says "Reading" or "Conversation" and names the scene; so are reviews,
...
/**
 * A vocabulary row's word count is the number of words it teaches.
 *
 * The row under a vocabulary title used to sample three of its words, which a
 * learner read as "this lesson has three words" or as three words thrown
 * together at random. It now states the count ("12 new words · part 1 of 2";
 * see `wordsLine` in src/data/units.ts). A count is only worth showing if it
 * is right, so this compares it with the ids the lesson actually owns.
 */
const miscounted = [];
for (const unit of UNITS) {
  for (const lesson of unit.lessons) {
    if (lesson.kind !== 'vocab') continue;
    const said = Number((/^(\d+) new words?\b/.exec(lesson.subtitle || '') || [])[1]);
    const owned = (lesson.wordIds || []).length;
    if (said !== owned) miscounted.push(`${unit.id} "${lesson.title} · ${lesson.subtitle}" teaches ${owned} words`);
  }
}
...
if (miscounted.length) {
  console.error(`\n${miscounted.length} vocabulary row(s) whose word count is wrong:`);
  for (const t of miscounted.slice(0, 25)) console.error(`  ${t}`);
  console.error(`\nThe count is generated by wordsLine in src/data/units.ts. Fix that, not the number.`);
}

if (gaps.length || stale.length || miscounted.length) process.exit(1);

console.log(
  '  Every topic a unit teaches is named by the unit that holds it, every review id is its unit,\n' +
    '  and every vocabulary row states the number of words it teaches.'
);
